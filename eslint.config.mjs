...
const settings = {
  ...
  'import/parsers': {
    '@typescript-eslint/parser': ['.ts', '.tsx', '.mts', '.cts', '.vue']
  },
  'import/resolver': {
    typescript: {
      alwaysTryTypes: true,
...
const eslintRules = {
  ...
  'import/no-unresolved': [ // 确保导入的模块可以解析为本地文件系统上的模块
    ...
    {
      commonjs: true,
      ignore: ['filenamify/browser', '@weibo-download-tools', '@weibo-download-tools-api']
    }
  ]
};
...
export default [
  ...
  },
  {
    ...
    languageOptions: {
      ...
      parserOptions: typescriptLanguageOptions,
      globals: languageGlobalsOptions
    },
    settings,
    plugins: {
